...
class OrganizationController {
  ...
  async donate({ request, response, view }) {
    ...
    donor_options.sort((a,b)=> b.balance - a.balance)
  ...
  _distributeRandomly(recipients, donation) {
    ...
    if (recipients.length === 1) {
      ...
      recipients[0].balance = currency(recipients[0].balance).add(remaining_donation).format();
    ...
    for (let i = 0; i < recipients.length; i++) {
      recipients[i].balance = currency(recipients[i].balance).add(dist_array[i].value).format();
    }
    return;
  }
